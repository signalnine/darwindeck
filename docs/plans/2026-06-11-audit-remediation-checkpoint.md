...
- **Task 29 (republish + close the loop): COMPLETE.** README/checkpoint tell the honest exit; the algorithm-comparison table is filled from the experiment matrix (below). CLAUDE.md/ROADMAP truth-pass for the honest exit can fold into a later commit.

**Experiment matrix (Task 29 step 2): COMPLETE (truncated N), 2026-06-13.** Ran on anarres on the final frozen stack; stopped at 20/60 runs (baseline 6, novelty 5, map-elites 4, random 5 of 15 each) when the host was taken down for security patching. The harness writes results.json only on full completion, so the authoritative record is the per-run Done: log, committed at results/2026-06-13-experiments-final/raw-run-log.txt with meta.json; the README table + exact Mann-Whitney are reproducible from it via the analysis in that commit. RESULT (coverage, complete separations all): novelty 0.198 > map-elites 0.125 > random 0.084 > baseline 0.050; novelty & MAP-Elites beat the random null (U at extreme, exact p=0.008), and BASELINE (fitness-sharing only) is significantly WORSE than random (U=0, p=0.004), same ordering on QD-score. HEADLINE: diversity machinery is necessary to beat random sampling of the constrained space; plain fitness sharing -- v1's family of approach -- explores it worse than random. Truncation does not change the ranking (every comparison is a no-overlap separation). NOTE: pairwise distance is omitted from the headline as misleading (baseline posts high pairwise ~0.48 with low coverage -- few cells, far apart).

**What remains (carried-forward research directions, for a FUTURE plan -- the frozen stack does not change):**
- **Optional re-run of the experiment matrix at full N=15** when a host is free, to tighten the (already-conclusive) intervals; the truncated N=4-6 result is committed and the ranking is settled.
