...
def test_validate_card_count_valid():
    """Valid card count passes validation."""
    ...
    assert validate_card_count(genome)


def test_validate_card_count_invalid():
    """Too many cards per player fails validation."""
    from darwindeck.evolution.operators import validate_card_count, STANDARD_DECK_SIZE
    from darwindeck.genome.examples import create_war_genome

    genome = create_war_genome()
    # War has 26 cards per player, 26 * 4 = 104 > 52
    assert not validate_card_count(genome, max_players=4)


def test_tweak_parameter_clamps_cards():
    """TweakParameterMutation should clamp cards_per_player."""
    from darwindeck.evolution.operators import TweakParameterMutation, STANDARD_DECK_SIZE, validate_card_count
    from darwindeck.genome.examples import create_war_genome
